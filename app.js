...
const simulatorConfigs = {
  TARIFFS: {
    ...
    multiOptions: ["China", "EU", "Mexico", "India", "Russia"],
  ...
  "INTEREST RATES": {
    ...
    multiOptions: ["United States", "Euro Area", "Japan", "Brazil", "Turkey"],
  ...
  SANCTIONS: {
    ...
    multiOptions: ["Russia", "Iran", "North Korea", "Myanmar", "Belarus"],
  ...
  TAXATION: {
    ...
    multiLabel: "Affected blocs",
    multiOptions: ["Households", "Exporters", "Banks", "Energy firms", "Technology firms"],
    defaultTargets: ["Technology firms"]
  },
  IMMIGRATION: {
    ...
    multiLabel: "Sector exposure",
    multiOptions: ["Construction", "Agriculture", "Healthcare", "Technology", "Hospitality"],
    defaultTargets: ["Construction"]
  },
  "INFLATION TARGET": {
    ...
    multiOptions: ["United States", "Euro Area", "United Kingdom", "Japan", "Brazil"],
    defaultTargets: ["United States"]
  }
};
...
function renderCountries() {
  ...
  app.innerHTML = `
    <section class="shell search-bar section">
      ...
      <input id="country-search" class="bare-input country-search" type="search" placeholder="Search 195 countries..." aria-label="Search countries">
  ...
  function selectCountry(country) {
    if (!countryProfiles[country]) return;
    activeCountry = country;
    activeDossierTab = "ECONOMY";
    document.querySelectorAll("[data-country]").forEach((chip) => chip.classList.toggle("is-selected", chip.dataset.country === country));
  ...
  document.getElementById("country-search").addEventListener("keydown", (event) => {
    ...
    const match = quick.find((country) => country.toLowerCase().startsWith(value));
...
function riskWorldMap() {
  const highlighted = [
    ...
    ["Ukraine", "ACTIVE CONFLICT", "The war remains the central European security shock and a continuing drain on fiscal, energy, and defense capacity. Reconstruction needs and export-route vulnerability keep the economic risk trajectory elevated.", "M494 174 L532 168 L552 188 L525 207 L489 198 Z"],
    ["China", "HIGH TENSION", "Strategic competition is concentrated in semiconductors, maritime claims, and export controls. Domestic demand weakness increases external pressure through industrial overcapacity and trade-defensive responses abroad.", "M665 202 L707 185 L750 201 L754 246 L718 273 L666 255 L642 224 Z"],
    ["Taiwan", "CONTESTED TERRITORY", "Taiwan links sovereignty risk with the most advanced semiconductor supply chain in the global economy. Coercive military activity below the war threshold still raises insurance, inventory, and relocation costs.", "M760 238 L768 250 L764 265 L754 253 Z"],
    ["Iran", "SANCTIONS REGIME", "Iran's sanctions-adapted economy remains tied to oil exports, regional networks, and maritime leverage. Escalation risk transmits through energy prices, shipping insurance, and compliance exposure.", "M552 238 L584 227 L611 248 L601 279 L564 282 L541 260 Z"],
    ["Israel and Palestinian Territories", "ACTIVE CONFLICT", "The conflict has produced regional escalation risk across energy, shipping, and alliance politics. The economic channel is less local output than the probability of wider confrontation.", "M526 250 L536 249 L542 272 L531 284 L520 269 Z"],
    ["Yemen", "ACTIVE CONFLICT", "The conflict and Red Sea attacks have turned a local war into a global shipping-risk variable. Rerouting around the Cape raises transit times, insurance premia, and working-capital requirements.", "M546 307 L579 304 L594 322 L568 340 L538 330 Z"],
    ["Sudan", "ACTIVE CONFLICT", "Civil war has disrupted state capacity, food security, and regional migration flows. The spillover risk is concentrated in the Horn of Africa, Red Sea logistics, and humanitarian financing.", "M506 287 L544 283 L562 321 L540 362 L498 341 L486 309 Z"],
    ["Myanmar", "POLITICAL INSTABILITY", "Civil conflict and military rule have fragmented territorial control and trade governance. The risk channel runs through energy projects, border commerce, and China's regional security calculus.", "M690 276 L714 264 L728 301 L711 338 L684 323 Z"],
    ["North Korea", "HIGH TENSION", "Missile development and sanctions evasion keep Northeast Asian security risk structurally elevated. Economic exposure is limited directly but high through alliance signaling and proliferation risk.", "M733 182 L751 180 L762 193 L747 207 L729 199 Z"],
    ["Syria", "SANCTIONS REGIME", "The conflict is frozen rather than resolved, leaving reconstruction, sanctions relief, and refugee return politically blocked. Regional normalization has not restored broad economic access.", "M516 239 L539 236 L551 248 L535 260 L512 253 Z"],
    ["Venezuela", "SANCTIONS REGIME", "Energy sanctions, debt distress, and migration pressure shape Venezuela's regional impact. Partial oil-sector relief can change supply margins without resolving institutional risk.", "M283 310 L317 305 L338 326 L324 354 L285 348 L267 327 Z"],
    ["Sahel", "POLITICAL INSTABILITY", "Military coups, insurgency, and foreign-security realignment have weakened regional coordination. The economic impact appears in mining risk, migration routes, and aid conditionality.", "M444 284 L493 277 L515 303 L500 333 L450 330 L421 305 Z"],
    ["South China Sea", "CONTESTED TERRITORY", "Competing maritime claims create a persistent low-intensity risk to trade routes and naval signaling. The core economic risk is escalation near logistics lanes that carry a large share of global goods trade.", "M705 278 L780 275 L800 320 L735 337 Z"]
  ];

  return `
    <div class="world-risk-wrap">
      <svg class="world-risk-map" viewBox="0 0 1000 560" role="img" aria-label="Global geopolitical risk map">
        <path class="map-land" d="M72 132 L125 88 L205 78 L282 112 L327 170 L304 235 L248 251 L219 309 L170 296 L130 245 L84 218 Z"></path>
        <path class="map-land" d="M280 298 L326 326 L361 395 L346 481 L295 520 L255 461 L237 374 Z"></path>
        <path class="map-land" d="M386 119 L479 78 L618 75 L763 95 L904 135 L954 192 L884 235 L747 217 L657 264 L542 238 L442 250 L371 197 Z"></path>
        <path class="map-land" d="M455 257 L523 245 L590 291 L610 374 L573 472 L505 482 L452 413 L425 326 Z"></path>
        <path class="map-land" d="M748 363 L826 344 L892 384 L882 446 L794 454 Z"></path>
        <path class="map-land" d="M438 105 L458 91 L489 98 L497 122 L468 135 L441 125 Z"></path>
        ${highlighted.map(([name, tier, note, d]) => `<path class="risk-country" data-name="${name}" data-tier="${tier}" data-note="${note}" style="fill:${riskTierColors[tier]}" d="${d}"></path>`).join("")}
      </svg>
